...
Before the first generation in a session, call `get_credit_balance` once to verify MCP auth is working.

If auth fails (401/403 or missing token), guide the user to connect:

1. Tell user SpriteCook requires API auth to use MCP tools
2. Offer guided setup (recommended):
...
3. Or manual setup docs: `https://www.spritecook.ai/api-docs`
4. Reload Cursor
5. Re-run `get_credit_balance`

When setup is complete, continue with asset generation workflow.
